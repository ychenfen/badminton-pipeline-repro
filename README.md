...
以下数字全部来自本仓库的实跑记录（`HANDOVER.md`），样本为一段 30 秒短视频（960×544，21 fps，630 帧）。

### 球检测：阈值的影响

TrackNet 输出热力图后需要二值化。原版硬编码阈值 0.5，在本视频上**一帧球都检不出**。
用 `scripts/tools/diag_tracknet.py` 在 4 个采样帧上实测：
...
阈值 0.15 时 598/630 帧检出球。需要说明的是，**这是"检出帧占比"而非对标注真值的准确率**——
球被网柱遮挡或飞出画面的帧本就应当是 invisible，本仓库没有做逐帧人工标注，因此无法给出 precision/recall。

### 球员速度：一处被指标掩盖的 bug

原实现用固定阈值判断 ByteTrack 是否串了 ID：位移 > 1.2 m 的帧间跳变视为无效。
但 21 fps 下 dt = 0.048 s，这个阈值实际放行的速度上限是 1.2 / 0.048 = **25 m/s**。
羽毛球运动员极限冲刺约 7 m/s，于是偶发的 ID 串位被当成真实位移记入 `rally_max_speed`，
面板显示"最高速度 24 m/s"——比博尔特还快。

改为按帧率动态计算 `max_jump_m = 8.0 * dt + 0.05` 后，同一视频同一帧的对比：

| 指标 | 固定阈值 1.2 m | 动态阈值 8×dt+0.05 |
|---|---:|---:|
...
| 阶段 | 30 s 短样本（630 帧） | 全长（13344 帧） | MPS 加速 |
|---|---|---|---|
| TrackNet 球检测 | ~100 s（CPU 0.16 s/帧） | ~3 小时 | 不支持，代码只走 CPU/CUDA |
| Overlay 球员分析 | — | ~30 分钟 | ~10 分钟 |
| FX 子弹时间 | — | ~5 分钟 | 不依赖 GPU |
...
- **检出率不等于准确率。** 94.9% 是检出帧占比，本仓库未做逐帧人工标注，
  没有 precision / recall / mAP 这类对真值的指标。要做严肃评测需要自建标注集。
- **阈值需按视频调。** 0.15 是在这一段视频上试出来的，换机位、换光照、换画质都可能要重调。
  原版默认 0.5 会静默失败（0% 检出且不报错），这是本项目踩过的最大的坑。
- **球场角点靠手工标。** `select_court.py` 需要人工点 4 个角，没有做自动球场检测，
  换视频就要重标一次。
- **ID 串位只是缓解，没有根除。** 动态阈值把虚假高速压下去了，
  但 ByteTrack 在两名球员交叠时仍会偶发串 ID，只是不再污染速度统计。
- **泛化性未验证。** 全流程只在一段 30 秒样本 + 一段全长视频上跑通，
  没有跨视频、跨机位、跨赛事的批量验证。
- **TrackNet 无 MPS 路径。** 全长视频在 Apple Silicon 上仍需 CPU 跑约 3 小时。
